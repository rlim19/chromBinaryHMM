...
class TestBaumWelch(unittest.TestCase):
   ...
   def test_NonNormalizedTransEstimate(self):
      ...
      alpha = np.array([[0.4,0.079,0.09396], [0.15,0.1085,0.021015]])
      beta = np.array([[0.189,0.7,1], [0.2625,0.55,1]])
      trans = np.array([[0.8,0.2], [0.5,0.5]])
      emm = np.array([[0.2,0.8],[0.7,0.3]])
      no_states = 2
      obs = np.genfromtxt("profile.test", names=True, delimiter="\t")
      es_trans = transEstimate( alpha, beta, trans, emm, no_states, obs['001'])
      target_est = np.matrix([[ 0.72849503, 0.27150497],[ 0.54416961, 0.45583039]])
